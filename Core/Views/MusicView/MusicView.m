...
- (void)togglePlayback {
    ...
    [self updatePlayButton:[controller isPlaying]];
    if (![controller _nowPlayingInfo]) { // check if music is already playing
        [controller playForEventSource:0];
    } else {
        [controller togglePlayPauseForEventSource:0];
    }
...
- (void)updatePlayButton:(BOOL)isPlaying {
    if (isPlaying) {
        UIImage* pauseImage = [[UIImage systemImageNamed:@"pause.fill"] imageWithConfiguration:[UIImageSymbolConfiguration configurationWithPointSize:65]];
        [[self playButton] setImage:pauseImage forState:UIControlStateNormal];
    } else {
        UIImage* playImage = [[UIImage systemImageNamed:@"play.fill"] imageWithConfiguration:[UIImageSymbolConfiguration configurationWithPointSize:65]];
        [[self playButton] setImage:playImage forState:UIControlStateNormal];
    }
}
@end